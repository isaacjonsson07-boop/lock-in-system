    ...
    <div className="max-w-4xl mx-auto">
                  ...
                  <div className="w-full bg-sa-bg rounded-sm relative" style={{ height: '60px' }}>
                    <div
                      ...
                      style={{
                        ...
                        background:
                          day.percentage === 100 ? 'var(--green)'
                          : day.percentage >= 80 ? 'var(--gold)'
                          : day.percentage > 0 ? 'rgba(201,169,110,0.4)'
                          : 'transparent',
                      }}
                    />
                  </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  <span className="text-sm text-sa-cream-soft w-36">{label}</span>
                  <div className="flex-1 sa-progress-track">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                      style={{ width: `${rate}%`, background: rateBarColor(rate) }}
                    />
                  </div>
                  <span className={`text-xs font-medium w-10 text-right ${rateColor(rate)}`}>{rate}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'weekly' && (
        <div className="space-y-6 animate-rise delay-2">
          ...
          <div className="sa-card-elevated">
            <p className="sa-section-subtitle text-sa-gold mb-3">This Week's Data</p>
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Non-Neg.', value: `${weekStats.nnRate}%` },
                { label: 'Habits', value: `${weekStats.habitRate}%` },
                { label: 'Tasks', value: `${weekStats.taskRate}%` },
              ].map(({ label, value }) => (
                <div key={label} className="text-center">
                  <p className="text-lg font-light text-sa-cream">{value}</p>
                  <p className="text-[0.55rem] text-sa-cream-faint uppercase tracking-wider mt-0.5">{label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <button
            onClick={() => handleSaveReview('weekly')}
      ...
      {activeTab === 'monthly' && (
        <div className="space-y-6 animate-rise delay-2">
          ...
          <button
            onClick={() => handleSaveReview('monthly')}
